...
const ConnectWalletModal = ({ isOpen, onClose, children }) => {
    ...
    async (contractAddress, contractAbi) => {
      try {
        ...
        return tokenIds.map(id => ({
          id,
          source: contractAddress === tradeContractAddress ? 'trade' : 'forge',
        }));
      } catch (error) {
  ...
  const fetchUserListings = useCallback(async () => {
    ...
    const forgeListings = await fetchListings(
  ...
  return (
    <>
      <div className="connect-modal-overlay" onClick={onClose}>
        ...
        >
          <button className="connect-close-button" onClick={onClose}>
            X
          </button>
          <div className="connect-modalContent-header">{children}</div>
          <div className="unlist-modalContainer">
            ...
            <div className="unlist-display-container">
              {entities.length > 0 ? (
                entities.map(entity => (
                  ...
                  >
                    <img
                      src={`https://example.com/${entity.id}.png`}
                      alt={entity.id}
                    />
                  </div>
                ))
            ...
            {selectedEntity && (
              <button className="unlist-button" onClick={unlistEntity}>
                Unlist
              </button>
            )}
          </div>
