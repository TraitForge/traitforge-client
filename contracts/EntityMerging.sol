...
contract EntityMerging is CustomERC721, IEntityMerging {
    ...
    function listForBreeding(uint256 tokenId, uint256 fee) public {
        ...
        require(breedingCounts[tokenId] < forgePotential, "Entity has reached its breeding limit");
    ...
    function breedWithListed(uint256 forgerTokenId, uint256 mergerTokenId) external payable nonReentrant returns (uint256) {
    require(listings[forgerTokenId].isListed, "Forger's entity not listed for breeding");
   ...
   function getEntropiesForTokens(uint256 forgerTokenId, uint256 mergerTokenId) 
        public view override returns (uint256 forgerEntropy, uint256 mergerEntropy) {
