...
function stake(uint256 _amount) external nonReentrant {
    ...
    uint256 pending = stakedBalances[msg.sender] * accTokenPerShare / 1e12 - rewardDebt[msg.sender];
    if (pending > 0) {
    ...
    totalStaked += _amount;
    rewardDebt[msg.sender] = stakedBalances[msg.sender] * accTokenPerShare / 1e12;

    emit Staked(msg.sender, _amount); // Corrected the typo in event call
}

function unstake(uint256 _amount) external nonReentrant {
    require(stakedBalances[msg.sender] >= _amount, "Unstaking amount exceeds balance"); // Fixed the typo in the require 
    require(stakedBalances[msg.sender] >= _amount, "Unstaking amount exceeds balance");
    require(block.timestamp >= stakeTimestamp[msg.sender] + LOCK_UP_PERIOD, "Tokens are locked");
...
function updateEntropyForAddress(address minter, uint256 entropyValue) external onlyOwner {
...
function distributeAirdropToAddress(address recipient) external onlyOwner nonReentrant {
    ...
    uint256 recipientTokens = (recipientEntropy * totalAirdropAmount) / totalEntropy;
    
    require(token.balanceOf(address(this)) >= recipientTokens, "Insufficient tokens for distribution");
